...
The whole toolchain is declared in [`mise.toml`](mise.toml) and installed by [mise](https://mise.jdx.dev): Node 24, Python 3.14, GraalVM CE 21 (with `native-image`), Kotlin 2.4 and Gradle 9, Rust 1.97, uv, the GitHub CLI, the AWS CLI, plus the agent tooling — [Gortex](https://github.com/zzet/gortex) (code-intelligence graph, indexed automatically on first boot), opencode and rtk. Every **version** lives in one file, [`.devcontainer/.env`](.devcontainer/.env), which both Docker Compose and mise read; `mise.lock` then pins each tool to an exact release asset and SHA256 per platform, so two people building the container get byte-identical tools. Rust is the one exception — mise delegates it to rustup, which fetches toolchains itself, so its lock entry carries a version but no checksums; `scripts/verify-mise-lock.mjs` names that exemption rather than hiding it, and `pnpm verify:mise` prints it on every run. The Claude Code CLI is installed separately (see [AGENTS.md](AGENTS.md#toolchain-mise) for why).
...
> 🧭 **Start at <http://localhost:8081>** — a dashboard listing everything this container runs, with a live up/down dot per service. It's a container, not an editor plugin, so it works in Codespaces and any browser. New services appear by adding them to `forwardPorts` in [`.devcontainer/devcontainer.json`](.devcontainer/devcontainer.json) and running `pnpm dashboard:sync` — that works for any language, not just Node.
