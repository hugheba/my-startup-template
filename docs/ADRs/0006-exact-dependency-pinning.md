...
Enforced by `pnpm verify:deps` ([`scripts/verify-exact-deps.mjs`](../../scripts/verify-exact-deps.mjs)), which fails CI on any loose specifier in any workspace `package.json`, in the `overrides` values in `pnpm-workspace.yaml`, and in `npx`/`dlx` invocations inside package scripts.
